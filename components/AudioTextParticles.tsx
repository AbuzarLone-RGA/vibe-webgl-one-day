...
const DAMPING             = 0.65
const BEAT_STRENGTH       = 0.10

const TRANSITION_START    = 9.5
...
export default function AudioTextParticles({ dataRef, analyserRef }: Props) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: /* glsl */ `
        ...
        void main() {
          ...
          // Bass pulses text particle size
          gl_PointSize = size * (400.0 / -mvPosition.z) * (1.0 + uBass * 1.2);
  ...
  useFrame((_, delta) => {
    ...
    if (analyser && dataRef.current) {
      // dataRef is already filled by AudioParticleSystem this frame
    ...
    const beatFired = bass - prevBass.current > 0.12
    ...
    for (let i = 0; i < TEXT_PARTICLE_COUNT; i++) {
      ...
      // Beat impulse — radial kick from text center
      if (beatFired) {
        const bd = Math.sqrt(localX * localX + localY * localY) || 1
        velocities[i3]     += (localX / bd) * BEAT_STRENGTH
        velocities[i3 + 1] += (localY / bd) * BEAT_STRENGTH
      }
